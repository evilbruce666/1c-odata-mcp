...
55 инструментов (21 чтение/аналитика + 34 записи). У всех есть необязательный параметр **`database`** (какая база 1С — см. `list_databases`); у аналитических — ещё и **`organization`** (фильтр по юрлицу — см. `list_organizations`).
...
| `create_services_act` | Акт об оказании услуг (доходы/расходы по номенклатурной группе) |
| `create_payment` / `create_payout_order` | Оплата от покупателя (поступление на р/с) / платёжное поручение |
| `create_bank_writeoff` | Списание с расчётного счёта (исходящий платёж, вид операции обязателен) |
| `create_cash_receipt` / `create_cash_payment` | Приходный / расходный кассовый ордер (ПКО / РКО) |
| `create_issued_invoice` / `create_received_invoice` | Счёт-фактура выданный / полученный (на основании реализации / поступления) |
...
- **Регламентные операции не создаются.** Закрытие месяца, амортизацию, расчёт себестоимости/НДС генерирует обработка «Закрытие месяца» своими алгоритмами — через OData их не запустить. Читать (`search_documents`/`get_document`) можно.
